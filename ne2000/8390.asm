...
ifndef	mkle8259		; Include extra per-type data
	...
	out	10h,al		; secondary 8259 for FUJITSU FM TOWNS
	pause_
	in	al,10h		; get it
	mov log[bx].le_irr2, al
	mov	al,0bh		; read in-service register from
	out	10h,al		; secondary 8259
	pause_
	in	al,10h		; get it
	mov log[bx].le_isr2, al
	mov	al,0ah		; read request register from
	out	00h,al		; primary 8259 for FUJITSU FM TOWNS
	pause_
	in	al,00h		; get it
	mov log[bx].le_irr1, al
	mov	al,0bh		; read in-service register from
	out	00h,al		; primary 8259
	pause_
	in	al,00h		; get it
...
;Step 14. Program EN_CCMD back to page 0, and start it.
  ...
  if 0
	...
	jb	set_int_num		; No.
	add	al, 70h - 8 - 8		; Map it to the real interrupt.	PC/AT 70h-77h is IRQ8-15
set_int_num:
