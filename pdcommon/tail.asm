...
; FM towns don't have PCI BIOS

;
; Get the feature byte (if reliable) so we can know if it is a NEC PC-9821
; computer and how many interrupts there are.
;

	mov	sys_features,TWO_8259	; FM TOWNS have 2nd 8259
...
;
; Map IRQ 7 to IRQ 9 if needed.
;
	test	sys_features,TWO_8259	; 2nd 8259 ?
	je	no_mapping_needed	;no, no mapping needed
	cmp	int_no,7		;map IRQ 7 to IRQ 9.
...
have_processor:
	...
	; FM TOWNS pcmcia not support
	mov	dx,offset cbus_msg
have_bus:
	mov	ah,9
	int	21h
